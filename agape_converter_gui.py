...
CSV_COLUMNS = [
    'FamilyID', 'Title', 'FirstName', 'MiddleName', 'LastName', 'Suffix',
    'Gender', 'Envelope', 'Address1', 'Address2', 'City', 'State', 'Zip',
    'Country', 'HomePhone', 'WorkPhone', 'MobilePhone', 'Email', 'WorkEmail',
    'BirthDate', 'MembershipDate', 'WeddingDate',
    'PersonCustom:Married:', 'FamilyCustom:CustomField1',
]
...
def convert_pdf(pdf_path, family_id=1001):
    ...
    last_name    = f.get('family_last_name', '').strip()
    address1     = f.get('address1', '').strip()
    address2     = f.get('address2', '').strip()
    city         = f.get('city', '').strip()
    state        = f.get('state', '').strip()
    zip_code     = f.get('zip', '').strip()
    home_phone   = normalise_phone(f.get('home_phone', ''))
    wedding_date = f.get('wedding_date', '').strip()
    notes        = f.get('notes', '').strip()
    rows = []

    head_first = f.get('head_first', '').strip()
    if head_first:
        rows.append(person_row(
            family_id, 'Mr', head_first, f.get('head_middle','').strip(),
            last_name, 'Male', f.get('head_birthdate',''),
            f.get('head_email','').strip(),
            normalise_phone(f.get('head_mobile','')),
            address1, address2, city, state, zip_code, home_phone, wedding_date,
            family_custom=notes,
        ))

    spouse_first = f.get('spouse_first', '').strip()
    if spouse_first:
        rows.append(person_row(
            family_id, 'Mrs', spouse_first, f.get('spouse_middle','').strip(),
            last_name, 'Female', f.get('spouse_birthdate',''),
            '', '', address1, address2, city, state, zip_code, home_phone, wedding_date,
        ))

    for i in range(1, 9):
        p = f'child{i}'
        c_first = f.get(f'{p}_first', '').strip()
        if not c_first:
            continue
        rows.append(person_row(
            family_id, '', c_first, f.get(f'{p}_middle','').strip(),
            last_name, '', f.get(f'{p}_birthdate',''),
            '', '', address1, address2, city, state, zip_code, home_phone, '',
            married_custom=f.get(f'{p}_spouse','').strip(),
        ))
    return rows, last_name
